...
(() => {
  ...
  const CONFIG = {
    ...
    CACHE_DURATION: 1 * 60 * 60 * 1000, // Cache should be refreshed every hour
  ...
  function convertText(text, dict) {
    ...
    // Handle words with apostrophes - sort by length to handle longer words first
    ...
    // Handle single words using a regex that includes a range of Latin characters with diacritics.
    result = result.replace(/\b[a-zA-Z\u00C0-\u024F0-9_]+\b/g, match =>
      dict.words[match.toLowerCase()] || match
    );
    // Handle special cases for 'ک' and 'د', plus linking isolated 'ال' with following words
  ...
  async function convertPage(toJawi) {
    ...
    if (toJawi) {
      ...
      // Find all text nodes to convert
      const textNodes = [];
      ...
      // Process nodes in chunks with requestAnimationFrame
      const chunkSize = 50;
      let currentIndex = 0;
      let isProcessing = true;
      // Create a cancellable processing function
      const processNextChunk = () => {
        if (!isProcessing || currentScript !== "jawi") {
          return; // Stop processing if we switched away from Jawi
        }
  ...
  function setupControls() {
    ...
    scriptLi.innerHTML = createControlsHTML("rumi-jawi", "Penukar tulisan", [{
        value: "rumi",
        label: "Rumi",
        checked: currentScript === "rumi"
      },
      {
        value: "jawi",
        label: "Jawi",
        checked: currentScript === "jawi"
      }
    ]);
