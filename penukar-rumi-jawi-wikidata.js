...
(() => {
  ...
  const Converter = {
    async convert(toJawi) {
      ...
      if (toJawi) {
        ...
        document.querySelectorAll(
          ...
          element.innerHTML = replaceHamzaWithSpan(element.textContent);
        });
      } else {
        this.revertToRumi();
      }
    ...
    async convertToJawi() {
      ...
      State.title.innerHTML = replaceHamzaWithSpan(this.convertText(State.title.textContent, State.dictionary));
  ...
  `;
    ...
    setupControls() {
      ...
      scriptLi.innerHTML = this.createControlsHTML("rumi-jawi-ui", "Paparan tulisan", options);
      if (isMobile) {
  ...
  function initializeApp() {
    ...
    try {
      ...
      if (pendingScript) sessionStorage.removeItem("pendingScript");
