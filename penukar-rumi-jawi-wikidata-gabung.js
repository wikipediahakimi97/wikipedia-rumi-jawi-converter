...
* With unified options. Pilihan paparan: Jawi/Rumi
*/
...
(() => {
	...
	const UIManager = {
		...
		getScriptPreference() {
			...
			return persistentScript === "jawi" || (!persistentScript && userLang === "ms-arab");
		},
		async setupControls() {
			...
			await mw.loader.using('@wikimedia/codex').then((require) => {
				...
				const { CdxField, CdxRadio, CdxPopover, CdxProgressBar } = require('@wikimedia/codex');
				const isJawi = this.getScriptPreference();
				...
				const converterApp = Vue.createMwApp({
					components: { CdxField, CdxRadio, CdxPopover, CdxProgressBar },
					data() {
						return {
							showPopover: false,
							loading: false,
							triggerElement: null,
							selectedDisplay: isJawi ? "jawi" : "rumi",
							currentDisplay: isJawi ? "jawi" : "rumi",
							displayOptions: [
					...
					computed: {
						currentLabels() {
							return this.currentDisplay === "jawi" ? this.labels.jawi : this.labels.rumi;
						},
						currentLang() {
							return this.currentDisplay === "jawi" ? "ms-arab" : "ms";
						}
					},
					beforeUnmount() {
					...
					methods: {
						async handleDisplayChange() {
							...
							updateButtonText(isJawi);
							this.currentDisplay = isJawi ? "jawi" : "rumi";
						},
						onUpdate(value) {
						...
						>
							<div class="converter-popover-content" 
								...
								<cdx-field :is-fieldset="true">
									<template #label>
										<span :lang="currentLang">
											{{ currentLabels.displayOption }}
										</span>
									</template>
									<cdx-radio 
	...
	async function initializeApp() {
		...
		try {
			...
			State.dictionary = await DictionaryManager.fetch();
			const isJawi = UIManager.getScriptPreference();
			State.setScript(isJawi ? "jawi" : "rumi");
