...
(() => {
  ...
  const UIManager = {
    ...
    handleUILanguageChange(language) {
      new mw.Api().saveOption("language", language)
    ...
    setInitialUIState() {
      ...
      const radioInput = document.querySelector(`input[name="ui-language"][value="${
        currentLanguage === 'ms-arab' ? 'jawi-ui' : 'rumi-ui'
      }"]`);
      if (radioInput) {
        radioInput.checked = true;
      }
    ...
    setupRadioButtons() {
      try {
        this.setupStyles();
        const menuTarget = document.querySelector('.vector-menu-content ul, .menu');
        if (!menuTarget) {
        ...
        const converterControls = this.createConverterControls();
        const languageControls = this.createLanguageControls();

        if (document.body.classList.contains('skin-minerva-latest')) {
          let container = document.querySelector('.menu .converter-container');
          if (!container) {
            container = document.createElement('div');
            container.className = 'converter-container';
            document.querySelector('.menu').appendChild(container);
          }
          container.appendChild(converterControls);
          container.appendChild(languageControls);
        } else {
          menuTarget.appendChild(converterControls);
          menuTarget.appendChild(languageControls);
        }

        // Event handlers for script selection
        document.querySelectorAll('.cdx-radio__input[name="rumi-jawi"]').forEach(radio => {
          radio.addEventListener('change', e => this.handleScriptChange(e.target.value === 'jawi'));
        });

        // Event handlers for UI language selection
        document.querySelectorAll('.cdx-radio__input[name="ui-language"]').forEach(radio => {
          radio.addEventListener('change', e => this.handleUILanguageChange(e.target.value === 'jawi-ui' ? 'ms-arab' : 'ms'));
        });
